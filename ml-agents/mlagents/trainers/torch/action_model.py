...
class ActionModel(nn.Module):
    def __init__(
        ...
        self.det_action = det_action #nn.Linear(hidden_size, self.action_spec.continuous_size)
    ...
    def forward(
        ...
        """
        if self.det_action:
            actions = self.get_det_action(inputs, self.tanh_squash)
            return actions
        else:
            dists = self._get_dists(inputs, masks)
    ...
    def get_det_action(
        self, inputs: torch.Tensor, tanh_squash: bool = False,
    ) -> Tuple[AgentAction]:
        """
        The forward method of this module. Outputs the action, log probs,
        and entropies given the encoding from the network body.
        ...
        discrete_action: Optional[List[torch.Tensor]] = None
        # This checks None because mypy complains otherwise
        continuous_action = self._continuous_distribution.mu(inputs)
        if tanh_squash:
...
class DetActionModel(nn.Module):
    def __init__(
        self,
        hidden_size: int,
        action_spec: ActionSpec,
    ...
    def get_action_out(self, inputs: torch.Tensor, masks: torch.Tensor) -> torch.Tensor:
        ...
        continuous_out = self.tanh(self.det_action(inputs))
        action_out_deprecated = self.tanh(self.det_action(inputs))
        if self._clip_action_on_export:
            continuous_out = torch.clamp(continuous_out, -3, 3) / 3
            action_out_deprecated = torch.clamp(action_out_deprecated, -3, 3) / 3
